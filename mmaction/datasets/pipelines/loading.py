...
class SampleFrames:
    ...
    def __init__(self,
        ...
        self.multi_clip_sample = multi_clip_sample
        #random dropout frames
        self.segment_times = segment_times
        assert self.out_of_bound_opt in ['loop', 'repeat_last']

        if self.segment_times > 1:
            self.num_clips *= self.segment_times
    ...
    def _get_train_clips(self, num_frames):
        ...
        # if self.segment_times > 1:
        #     np.random.shuffle(clip_offsets)
        #     clip_offsets = clip_offsets[self.num_clips//2:]
        #     clip_offsets = np.sort(clip_offsets)
    ...
    def _get_test_clips(self, num_frames):
        ...
        if num_frames > ori_clip_len - 1:
            ...
            # elif self.multi_clip_sample:
            #     # print("2222222222")
            #     clip_offsets = base_offsets
            #     for i in range(self.multi_clip_sample-1):
            #         new_offsets = (base_offsets + (i+1) * avg_interval / float(self.multi_clip_sample)).astype(np.int)
            #         clip_offsets = np.concatenate([clip_offsets, new_offsets])
        else:
            clip_offsets = np.zeros((self.num_clips, ), dtype=np.int)
        ...
        if self.segment_times > 1:
            ...
            clip_offsets = clip_offsets[self.num_clips//2:]
            clip_offsets = np.sort(clip_offsets)
        # print("11111111", clip_offsets.shape, clip_offsets)
...
@PIPELINES.register_module()
class SnippetSampleFrames:
    """Snippet Sample frames from the video.
    """
    ...
    def __init__(self,
        ...
        if start_index is not None:
            warnings.warn('No longer support "start_index" in "SnippetSampleFrames", '
                          'it should be set in dataset class, see this pr: '
                          'https://github.com/open-mmlab/mmaction2/pull/89')
    ...
    def _get_train_clips(self, num_frames):
        ...
        if num_frames >= self.num_clips + self.new_length -1:
            base_offsets = np.arange(self.num_clips) * avg_interval
            clip_offsets = base_offsets + np.random.randint(
                  avg_interval, size=self.num_clips)
            ...
            clip_offsets = np.sort(
                np.random.randint(
                    num_frames - self.new_length + 1, size=self.num_clips))
        elif avg_interval == 0:
            ratio = (num_frames - ori_clip_len + 1.0) / self.num_clips
            clip_offsets = np.around(np.arange(self.num_clips) * ratio)
        else:
    ...
    def _get_shift_test_clips(self, num_frames, n_shift=3):
        if num_frames >= self.num_clips*n_shift + self.new_length - 1:
            avg_interval = (num_frames - self.new_length + 1)/float(self.num_clips)
            base_offsets = []
            clip_offsets = []

            # base_offsets = np.arange(self.num_clips) * avg_interval

            gap = int(avg_interval // n_shift)
            if (n_shift - 1) * (gap + 1) < int(avg_interval):
                gap = gap + 1
            else:
                gap = gap
            for start_i in range(0, gap*n_shift, gap):
                clip_offsets.extend([int(start_i + avg_interval*x) for x in range(self.num_clips)])
                if self.twice_sample:
                    base_offsets.extend([int(start_i + avg_interval / 2.0 + avg_interval*x) for x in range(self.num_clips)])
        ...
        elif num_frames >= self.num_clips + self.new_length -1:
            avg_interval = (num_frames - self.new_length + 1) / float(self.num_clips)
            clip_offsets = np.array(n_shift*[int(avg_interval / 2.0 + avg_interval*x) for x in range(self.num_clips)])

            #adddd
            if self.twice_sample:
                base_offsets = np.array(
                    n_shift * [int(avg_interval * x) for x in range(self.num_clips)])
                clip_offsets = np.concatenate([clip_offsets, base_offsets])


        elif num_frames >= self.num_clips:
            clip_offsets = np.array(n_shift*list(range(self.num_clips)))
        else:
            clip_offsets = np.tile(np.array(range(num_frames - self.new_length + 1)),
                            ((self.num_clips - self.new_length + 1) // (num_frames - self.new_length + 1), 1))
            clip_offsets = clip_offsets.flatten('F')
            clip_offsets = np.pad(clip_offsets, (0, self.num_clips - len(clip_offsets)), 'maximum')
            clip_offsets = np.tile(clip_offsets, n_shift)
        return clip_offsets
    ...
    def _sample_clips(self, num_frames):
        ...
        if self.test_mode:
            ...
            if self.shift_val:
                clip_offsets = self._get_shift_test_clips(num_frames, self.shift_val)
            else:
                clip_offsets = self._get_test_clips(num_frames)
        else:
            clip_offsets = self._get_train_clips(num_frames)
        ...
        #snippet_sample
        idx_list = []
        for seg_ind in clip_offsets:
            p = int(seg_ind)
            if p > num_frames:
                p = num_frames
            for i in range(self.new_length):
                idx_list = np.append(idx_list, p)
                if p < num_frames:
                    p += 1
        clip_offsets = idx_list
        #print("clip_offsets-ssss", clip_offsets)
    ...
    def __call__(self, results):
        ...
        frame_inds = frame_inds.reshape((-1, self.clip_len*self.new_length))
