...
from .ams_3D_module import CPTM_Bottleneck
try:
...
class Bottleneck3d(nn.Module):
    ...
    def __init__(self,
        ...
        self.embedding_mode = 'replace'
        self.insert_stage_flag = insert_stage_flag
        self.AMG_dim_ratio = AMG_dim_ratio 

        assert(self.embedding_mode in ['insert', 'replace'])

        # =======================================================
        # set AMS hyper-parameter here
        if self.embedding_mode == 'replace':
            # A1
            self.AMG_dim_restore = False
            self.AMG_conv1d_relu_out = False
            self.AMG_short_cut = True
        else : # insert
            self.AMG_dim_restore = True
            self.AMG_conv1d_relu_out = True
            self.AMG_short_cut = False
        # =======================================================
        ...
        if self.embedding_mode == 'replace' and afg_add_flag :
            self.conv1 = CPTM_Bottleneck(
                inplanes, 2, gamma=1,
                dim_restore=self.AMG_dim_restore,
                conv1d_relu_flag=self.AMG_conv1d_relu_out,
                short_cut=self.AMG_short_cut,
                percent=self.AMG_dim_ratio) 
        else :
            self.conv1 = ConvModule(
                inplanes,
                planes,
        ...
        if self.embedding_mode == 'insert' and afg_add_flag :
            self.conv3 = nn.Sequential(
                ...
                CPTM_Bottleneck(
                    ...
                    conv1d_relu_flag=self.AMG_conv1d_relu_out,
                    short_cut=self.AMG_short_cut) 
                    )# embedding into inherent shortcut!
        else :
...
class ResNet3dLayer(nn.Module):
    ...
    def __init__(self,
        ...
        super().__init__()
        self.arch_settings = AMSResNet3d.arch_settings
        assert depth in self.arch_settings

        self.make_res_layer = AMSResNet3d.make_res_layer
        self._inflate_conv_params = AMSResNet3d._inflate_conv_params
        self._inflate_bn_params = AMSResNet3d._inflate_bn_params
        self._inflate_weights = AMSResNet3d._inflate_weights
        self._init_weights = AMSResNet3d._init_weights
...
if mmdet_imported:
    ...
    MMDET_BACKBONES.register_module()(AMSResNet3d)
