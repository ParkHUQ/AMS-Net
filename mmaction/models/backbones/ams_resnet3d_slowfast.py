...
from .ams_3D_module import CPTM_Bottleneck as AFG_module
...
class AMSResNet3dPathway(AMSResNet3d):
    ...
    def inflate_weights(self, logger):
        ...
        for name, module in self.named_modules():
            if 'lateral' in name:
                continue
            if isinstance(module, ConvModule): # bottleneck
                ...
                if original_bn_name + '.weight' not in state_dict_r2d:
                    ...
                    self._inflate_bn_params(module.bn, state_dict_r2d,
                                            ...
                                            inflated_param_names)
                counter += 1
            elif isinstance(module, AFG_module) :
                aa = 1

                # initialization plan A1
                # self.init_A1(self, module, name, state_dict_r2d)
                
                # initialization idg02
                for name_mi, mi in module.named_modules():
                    if isinstance(mi, nn.Conv3d) or isinstance(mi, nn.Linear):
                        nn.init.kaiming_normal_(mi.weight, mode='fan_out', nonlinearity='relu')
                        print(name_mi + ' kaiming initialed')
                    elif isinstance(mi, nn.BatchNorm3d) or isinstance(mi, nn.BatchNorm1d):
                        nn.init.zeros_(mi.bias)

                        if 'temporal_blocks' in name_mi : #bn_zero_init
                            nn.init.zeros_(mi.weight)
                            print(name_mi + ' zeros initialed')
                        else:
                            nn.init.ones_(mi.weight)
                            print(name_mi + ' ones initialed')
                    elif mi.parameters() is None:
                        continue
                    else:
                        print('skip ' + name_mi)

                # pretrained weight initial for reduction layer
                old_k = name.replace('conv1.1','conv1')
                old_k_bn = old_k.replace('conv','bn')

                assert(module.reduce.weight.data.numel()  == state_dict_r2d[old_k + '.weight'].numel())
                module.reduce.weight.data = \
                    state_dict_r2d[old_k+'.weight'].reshape_as(module.reduce.weight)

                assert(module.bn_re.running_mean.numel() == state_dict_r2d[old_k_bn+'.running_mean'].numel())
                module.bn_re.running_mean = state_dict_r2d[old_k_bn+'.running_mean']
                module.bn_re.running_var = state_dict_r2d[old_k_bn+'.running_var']
                module.bn_re.weight.data = state_dict_r2d[old_k_bn+'.weight']
                module.bn_re.bias.data = state_dict_r2d[old_k_bn+'.bias']


        print('==='*40)
        print('53 pretrained convs, {} AFG module initialed, {} layers convs initialized as pretrained'.format(53-counter, counter)) 
        print('==='*40)
    ...
    def init_A1(self, module, name, state_dict_r2d):
                    # initialization plan A1
                # standard initial for all parameters
                for name_mi, mi in module.named_modules():
                    if isinstance(mi, nn.Conv3d) or isinstance(mi, nn.Linear):
                        nn.init.kaiming_normal_(mi.weight, mode='fan_out', nonlinearity='relu')
                        print(name_mi + ' kaiming initialed')
